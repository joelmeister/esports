...
$cookieName
	Example:  $_COOKIE[$cookieName] = $cookieValue;
				if(isset($_COOKIE[$cookieName])) {echo 'cookie is set';}
$cookieValue
	Example:  $cookieValue = $_COOKIE[$cookieName];
	
$cookieTime is in seconds.  
	-Default is to clear the cookie.
$cookiePath is the server path for which the cookie is available.  
	-Default is root "/"
*/
function easy_cookie($cookieName,$cookieValue,$cookieTime=-999999,$cookiePath="/"){
	$cookieTime += time(); //set timeout value
	
	setcookie($cookieName,$cookieValue,$cookieTime,$cookiePath,$_SERVER['SERVER_NAME']);
}
?>
